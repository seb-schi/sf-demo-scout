...
8b. **Determine modifications** — identify which elements to change (e.g., `<accessLevel>`, `<sharedTo>`, `<criteriaItems>`, `<label>`) and what the new values will be.

#### For Delete:

8a. **Locate the target rule** — find the rule by `<fullName>` or `<label>` in the existing `.sharingRules-meta.xml` file.

8b. **Count remaining rules** — run: `grep -c '<sharingCriteriaRules>\|<sharingOwnerRules>\|<sharingGuestRules>' <file>` to get the total rule count. If the count is 1 (only the rule being deleted), the file must be removed entirely in Phase 4.

### Phase 4 — Write and Verify

**Scout autonomous-build behavior (no interactive SE channel).** Upstream ships a "Phase 4 — Confirm
with User" gate here that stops and waits for a reply before any disk write. This skill runs inside a
Scout Phase-1 sub-agent that cannot prompt the SE mid-build — there is no one to reply, and stopping
to ask would stall the deployment. The gate is therefore **removed**: write the change directly and
record what was written in `discovery_notes`, so the SE reviews it in the change log / handover brief
rather than a blocking prompt. Scout's SE review happens *before* the build (spec approval during
sparring) and *after* it (change log) — never mid-build.

9. **Apply the change** — write it directly, no confirmation step:
...
| Record every write in `discovery_notes` (Scout autonomous mode) | No mid-build SE channel exists — the change log is the review surface, not a blocking prompt |
...
| File | When to read |
|------|-------------|
| `references/rule-types.md` | Phase 2 — before generating any rule, to get the complete XML schema for each rule type; also carries the composed Account owner-rule example |
