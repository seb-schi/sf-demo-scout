...
Full procedure lives under `<!-- IF:LRP -->` in `${CLAUDE_PLUGIN_ROOT}/prompts/building/phase1.md`. This summary is for orchestrator-level decisions and rollback.
...
2. Before generating, skim `${CLAUDE_PLUGIN_ROOT}/skills/sf-flow/references/xml-gotchas.md` — root-level alphabetical ordering, fault-connector self-reference, relationship-field traps in recordLookups, `storeOutputAutomatically` data-leak risk. Deployment blockers.
...
- Reusable patterns: `${CLAUDE_PLUGIN_ROOT}/skills/sf-flow/assets/subflows/` (bulk-updater, dml-rollback, email-alert, error-logger, query-with-retry, record-validator).
