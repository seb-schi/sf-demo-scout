...
## When & How — always compose, then write (Scout autonomous mode)

A `<description>` is **mandatory** — always compose one. Do it when **creating** the object
and again on **any** change: adding, updating, or deleting a field **or a validation rule**
(keeps it from going stale). Validation-rule changes count exactly like field changes — the
change isn't done until the description is refreshed. Never ask *whether* to add a
description.

**Scout runs this skill inside an autonomous Phase-1 sub-agent with no interactive channel
to the SE** — there is no one to reply to a mid-build prompt, and stopping to ask would
stall the build. So do NOT stop for approval of the wording. Compose the description and
write it, following the branch below:

- **No existing description (brand-new object):** compose (Steps 1–2) and write it. Nothing
  to preserve.
- **An existing description (update / delete / re-enrich):** treat the existing text as a
  **strong signal** — it may have been hand-authored by an admin and carry business context
  the schema can't reveal. Preserve that context and fold the new field/rule in rather than
  discarding it. Compose the enriched description, write it, and record the before/after in
  `discovery_notes` verbatim:
  `"Object description enriched on <Object>: '<old text>' -> '<new text>' (auto-applied — surfaced for SE review)"`
  so the SE sees the change in the change log and handover brief and can revert if they
  disagree. The change-log entry IS the SE's review surface — never block the build on a
