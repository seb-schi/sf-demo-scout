...
- **Add, update, or delete a field OR a validation rule on an existing object** — any of these may make the object's `<description>` stale, so refresh it (compose + write; see Section 3.B). This applies equally to validation-rule changes, not just fields.
...
> **🔔 Description freshness — applies to EVERY object change, fields AND validation rules:** Whenever you add, update, or delete a field **or a validation rule** on an object, the `<description>` may now be stale. Before finishing, refresh it per **Section 3.B** (compose + write). A validation-rule change counts exactly like a field change — the change is **not** done until the description has been reconciled. This is easy to forget on validation-rule edits/deletes — don't.
...
**Scout autonomous-build behavior (no interactive SE channel).** This skill runs inside a Scout Phase-1 sub-agent that cannot prompt the SE mid-build — there is no one to reply, and stopping to ask would stall the deployment. So **do NOT stop to ask for approval of the wording.** Compose the description and write it:

- **No existing description (brand-new object):** compose (steps below) and write it directly. Nothing to preserve.
- **An existing description (update / delete / re-enrich):** treat the existing text as a **strong signal** — it may have been hand-authored by an admin, carrying business context (domain, team, intent) the schema can't reveal. Preserve that context and fold the new field/rule in rather than discarding it. Compose the enriched description, write it, and record the before/after in `discovery_notes` verbatim: `"Object description enriched on <Object>: '<old>' -> '<new>' (auto-applied — surfaced for SE review)"`. The change-log entry that consumes `discovery_notes` IS the SE's review surface — they see the before/after in the handover and can revert if they disagree. **Never block the build on a description decision.**
...
- [ ] For an existing description (update/delete/re-enrich), recorded the before/after in `discovery_notes` for SE review — did NOT stop the build to ask for approval

### Architectural Checks
- [ ] Is `<description>` present? (Enriched per Section B — composed and written autonomously; existing-description changes recorded in `discovery_notes`.)
