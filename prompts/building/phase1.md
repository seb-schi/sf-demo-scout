...
- `service-email-to-case-configure` — Email-to-Case (org-wide `CaseSettings` + routing addresses) via the Metadata API, applied through the skill's own two-phase script (never a plain deploy) — invoke when the spec has an Email-to-Case section. Also the entry point for attaching an Agentforce service agent to an email routing address (delegates to `agentforce-generate` + `service-agentforce-channel-configure` Branch C after a capability probe).
...
3. **Folder handling — the one wrinkle.** A report must live in a folder (`reports/<FolderName>/<ReportName>.report-meta.xml`) with a `ReportFolder` metadata file (`reports/<FolderName>-meta.xml`) carrying `<folderShares>` (see the skill's Rule 14). If the spec's target folder does NOT yet exist in the org (`retrieve_metadata` for `ReportFolder` member `[FolderName]` returns nothing), author the folder file too and deploy `ReportFolder` + `Report` together. If the folder ALREADY exists, do NOT author or redeploy the folder file — deploy `Report` alone (redeploying an existing folder would overwrite its `<folderShares>`). Record which case applied in `discovery_notes`.
...
7. Rollback: `sf project delete source --metadata Report:[FolderName]/[ReportName] --target-org [alias]`. If this PLAN's step 3 deployed a NEW folder, also `sf project delete source --metadata ReportFolder:[FolderName] --target-org [alias]` (delete the report first). Do NOT delete a pre-existing folder.
<!-- /IF:REPORTS -->
...
<!-- IF:EMAIL_TO_CASE -->
### Email-to-Case Rules
Scope: org-wide Email-to-Case configuration (`CaseSettings` singleton + `emailToCase` routing addresses), authored and applied entirely via the Metadata API through the skill's own two-phase Python script — never a plain `deploy_metadata` call. Invoke the `service-email-to-case-configure` skill before authoring — it carries the full Required Inputs elicitation (routing emails, Support Settings preserve-first rules, optional toggles), the CaseSettings source-file template, and the Agentforce-delegation branch.
...
8. **Optional Agentforce delegation** — only if the spec's Agentforce section names this routing address as an agent channel: run `scripts/check-agent-email-capability.sh <alias>` first. Exit `3` → not entitled: skip the delegation, configure the base only, and record it in `issues`. Exit `0` → proceed via `agentforce-generate` (omit the Service Customer Verification topic — unsupported on the email channel; include an Escalation subagent so the agent can hand off a case to a human) then `service-agentforce-channel-configure` Branch C. Any other exit code is an unreachable-org condition, not "not entitled" — do not treat it as a decline.
