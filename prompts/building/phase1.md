...
This block is binding. Work only on its approved executable slice, honor every hard
exclusion during deployment and retry, and report excluded or out-of-scope work as
BLOCKED rather than expanding the phase. Explicit no-Apex remains binding even when
a standard action or metadata path fails.
...
- New whole Lightning Record Pages stay manual/App Builder. For a selected compatible field-section append, use only this phase's `IF:LRP` procedure; do not invoke a base-page generator.
- `platform-sharing-rules-generate` — record-level Sharing Rules (criteria / owner / guest) — invoke when the spec has a Sharing Rules section. NOTE: a sharing rule only grants access below a restrictive OWD; for a custom object set its `<sharingModel>` to `Private`/`Read` in the same deploy. For a standard object the OWD is an SE manual prerequisite — not because it is un-settable via metadata (it is settable, API 30.0+), but because a `CustomObject:<StandardObject>` deploy redeploys the entire object rather than one element, and a restrictive change triggers org-wide sharing recalculation. Never deploy a standard-object OWD change; record the dependency in `discovery_notes` if the spec flags the standard-object OWD as not-yet-set.
...
- `platform-report-generate` — Lightning Report (`.report-meta.xml`) authoring: Tabular / Summary / Matrix / Joined formats, columns, groupings, filters, charts, folder + `<folderShares>` — invoke when the spec has a Reports section. NOTE: this skill's frontmatter references `salesforce-api-context` MCP tools that are NOT in Scout's host — rely on the skill's static `references/column-names.md` tables for platform column names; do NOT attempt the MCP calls (see the `IF:REPORTS` block).
- `platform-custom-setting-generate` — Custom Setting metadata (a `CustomObject` with `<customSettingsType>` — Hierarchy or List): feature flags, kill switches, trigger-bypass toggles, per-profile/per-user config a Flow or Apex reads — invoke when the spec has a Custom Settings section.
- `platform-custom-metadata-type-generate` — Custom Metadata Type (`__mdt`) authoring: the type definition, fields (incl. `MetadataRelationship`), and deployable `.md-meta.xml` records — deployable reference / mapping / crosswalk config — invoke when the spec has a Custom Metadata Types section.
- `service-email-to-case-configure` — base Email-to-Case only (org-wide `CaseSettings` + routing addresses) via the Metadata API, applied through the skill's own two-phase script (never a plain deploy) — invoke when the spec has an Email-to-Case section. Agent creation and channel assignment belong to Phase 3.
...
2. File is `<DeveloperName>.reportType-meta.xml`; `<fullName>` MUST equal the DeveloperName (and the filename). One file per report type. For an existing report type, preserve this exact file under the shared component rollback contract before editing.
...
7. Rollback: restore an existing report type from its verified first `component-preedit` artifact. Delete only a report type whose prior absence was positively established, after the explicit destructive-action guard.
<!-- /IF:CUSTOM_REPORT_TYPE -->

<!-- IF:REPORTS -->
### Report Rules
Scope: `Report` metadata — a Lightning Report (`.report-meta.xml`) in a folder. Invoke the `platform-report-generate` skill before authoring — it carries the format→required-elements matrix, filter/grouping/chart shapes, the platform column-name tables, and the deployment-killer checklist. One report = one file; preserve an existing report's exact file under the shared component rollback contract before editing it.
1. Invoke `platform-report-generate` for the metadata shape, format rules, and the pre-generate deployment-killer checklist.
2. **MCP note — do NOT call `salesforce-api-context` tools.** The skill's frontmatter references `get_metadata_type_sections` / `get_metadata_type_context`; those tools are NOT in Scout's MCP host. Use the skill's static `references/column-names.md` tables for platform report column names (they cover the standard objects a demo reports on — Opportunity, Case, Lead, Account). Report columns use platform names, NOT raw API field names (e.g. `CLOSE_DATE` not `CloseDate`, `STAGE_NAME` not `StageName`) — a wrong column name is this skill's #1 deployment killer. For a column on an object/field NOT in the reference tables and not obvious from the pattern, issue one `salesforce_docs_search` (per `demo-docs-consultation`) before guessing; if still unresolved, mark the report BLOCKED before writing and surface the unresolved column in `issues`. Never fabricate an unavailable API-context call or treat the static table as live validation.
3. **Folder handling — the one wrinkle.** A report must live in a folder (`reports/<FolderName>/<ReportName>.report-meta.xml`) with a `ReportFolder` metadata file (`reports/<FolderName>-meta.xml`) carrying `<folderShares>` (see the skill's Rule 14). Classify the report and folder separately before mutation. Treat the folder as existing only after a successful exact retrieval and do not author or redeploy its file (that would overwrite `<folderShares>`). Treat it as new only from saved positive absence evidence for that exact `ReportFolder`; then author the folder and deploy `ReportFolder` + `Report` together. An empty, failed, ambiguous, or unavailable retrieval is unknown: mark the affected report/folder work BLOCKED, preserve any existing folder intact, and do not create or deploy a folder. Record the evidence and case in `discovery_notes`.
4. **Format rules (the skill enforces these — restated for the verify step):** Tabular = no groupings, no chart/aggregates; Summary = 1-3 `<groupingsDown>`; Matrix = 1-3 down + 1-3 across; Joined = 2-5 `<block>` (each Summary/Matrix). A grouping field must NEVER also appear in `<columns>` (automatic deploy failure). `<description>` max 255 chars; `<name>` max 40 chars.
5. Deploy `Report` (with `ReportFolder` first in the same deploy if the folder is new per step 3).
6. Verify (post-deploy read-back — do NOT skip): `retrieve_metadata` for `Report` member `[FolderName]/[ReportName]`, then assert against the retrieved XML — `<reportType>` equals the spec's report type; `<format>` matches; every column the spec names is present in `<columns>`; if the spec asked for groupings, the `<groupingsDown>`/`<groupingsAcross>` fields are present and do NOT also appear in `<columns>`. If any is missing, mark the report `FAILED` in your JSON output and surface it in `issues` verbatim.
7. Rollback: restore an existing report from its verified first `component-preedit` artifact. Delete only a report whose prior absence was positively established. A folder is a separate target: delete it only when its prior absence was positively established and current read-back proves it empty; preserve a pre-existing folder and any later-added or unknown contents.
<!-- /IF:REPORTS -->

<!-- IF:CUSTOM_SETTING -->
### Custom Setting Rules
Scope: a Custom Setting — a `CustomObject` with `<customSettingsType>` set (`Hierarchy` or `List`). Invoke the `platform-custom-setting-generate` skill before authoring — it carries the forbidden-element list, the supported field-type allowlist, and the wrong-component trap in step 2.
1. Invoke `platform-custom-setting-generate` for the metadata shape and field rules.
2. **`<customSettingsType>` is MANDATORY (`Hierarchy` or `List`) — this is the one trap.** Omitting it does NOT produce a clear error: the component silently validates as a regular custom object and can deploy green as the WRONG kind of component. If a deploy reports `Must specify a non-empty plural label for the CustomObject` on something the spec asked to be a custom setting, NEVER satisfy it by adding `<pluralLabel>` — that (plus `nameField`/`sharingModel`/`deploymentStatus`) makes the deploy succeed as a plain custom object the spec did not ask for. Add `<customSettingsType>` instead. Default to `Hierarchy` unless the spec describes a keyed reference table (then `List`).
3. File is `objects/<Name>__c/<Name>__c.object-meta.xml` (suffix `__c` — identical to a regular custom object; the suffix does NOT distinguish them). Fields are ordinary `CustomField` components under `objects/<Name>__c/fields/`. Forbidden on the setting (each fails `Cannot specify: <element> for CustomSettings`): `<pluralLabel>`, `<nameField>`, `<sharingModel>`, `<deploymentStatus>`, `<recordTypes>`, `<compactLayouts>`, `<validationRules>`, `<enableActivities/Reports/History/Search>`. Supported field types ONLY: `Checkbox`, `Currency`, `Date`, `DateTime`, `Email`, `Number`, `Percent`, `Phone`, `Text`, `TextArea`, `Url` — NO `Picklist`, NO `Formula`, NO `Lookup`/`MasterDetail` (all fail with the bare `Invalid data type.`; the skill's Section 4 is authoritative).
4. Deploy the setting (+ fields) via `deploy_metadata`.
...
Scope: a Custom Metadata Type (`__mdt`) — its type definition, fields, and deployable records. Invoke the `platform-custom-metadata-type-generate` skill before authoring — it carries the forbidden-element list, the field-type allowlist, the `MetadataRelationship` rules, and the full record-value `xsi:type` catalog (`references/cmdt-records.md`).
1. Invoke `platform-custom-metadata-type-generate` for the metadata shape, field rules, and record rules.
2. **Three artifact families.** Type: `objects/<Name>__mdt/<Name>__mdt.object-meta.xml` (root element `<CustomObject>`, but almost NO custom-object rules apply — requires `<label>` + `<pluralLabel>` + `<visibility>` + `<description>`; forbidden and hard-erroring: `<sharingModel>`, `<nameField>`, `<deploymentStatus>`, `<enableSearch>`, etc.). Fields: `objects/<Name>__mdt/fields/<Field>__c.field-meta.xml` (suffix `__c`; supported types per the skill — NO `Currency`, NO `Formula`; write `MetadataRelationship` EXPLICITLY, NEVER `Lookup`, which deploys green and is silently coerced to `MetadataRelationship`, churning git on the next retrieve). Records: `customMetadata/<TypeNameWithout__mdt>.<RecordDeveloperName>.md-meta.xml`.
3. **Records — the critical rules:** all three namespaces (`xmlns`, `xmlns:xsd`, `xmlns:xsi`) on the root `<CustomMetadata>`; every non-nil `<value>` carries an `xsi:type` (a missing one is fatal, even for text). **For a `Picklist` field's value emit `xsi:type="xsd:string"` — NEVER `xsi:type="xsd:picklist"`.** Salesforce's own docs say `xsd:picklist`; they are WRONG — it is not a valid schema type and fails the ENTIRE deploy server-side with no component-level diagnostics, taking every other component down with it. `<label>` is required on every record (omission gives `Required fields are missing: [MasterLabel]`). Never put `<fullName>` inside a `<values>` block. The full verbatim record-value catalog is in the skill's `references/cmdt-records.md`.
4. **Record DeveloperName derivation — deterministic, MANDATORY for batch/CSV/prose input.** Never free-hand a DeveloperName. Derive each one from its source label with this exact transform (fold every run of non-`[A-Za-z0-9]` to a single `_`, strip leading `_`, prefix `X` if it starts with a digit, strip trailing `_`, truncate to 40 chars then strip a trailing `_` the cut introduced; if empty, `Record_<ROW>`):
   ```bash
   printf '%s' "<LABEL>" | sed -E 's/[^A-Za-z0-9]+/_/g; s/^_+//; s/^([0-9])/X\1/; s/_+$//' | cut -c1-40 | sed -E 's/_+$//'
   ```
   (This is the algorithm in the skill's vendored `scripts/sanitize-developer-name.sh`; use the inline command above rather than the script path, because `${CLAUDE_PLUGIN_ROOT}` does not resolve in a phase sub-agent's shell.) The transform is many-to-one — `United States` and `United-States` both yield `United_States`. After deriving the whole batch, check for duplicate DeveloperNames (`sort | uniq -d`) BEFORE writing any file — on any collision STOP and report the colliding source labels rather than silently overwrite a record (a collision loses a row with NO deploy error). DeveloperName ≤ 40 chars, begins with a letter, no spaces/hyphens.
...
<!-- IF:EMAIL_TO_CASE -->
### Email-to-Case Rules
Scope: base Email-to-Case configuration only (`CaseSettings` singleton + `emailToCase` routing addresses), authored and applied entirely via the Metadata API through the skill's own two-phase Python script — never a plain `deploy_metadata` call. Invoke the `service-email-to-case-configure` skill before authoring; it carries the Required Inputs elicitation, Support Settings preserve-first rules, optional toggles, and source-file template.
1. Invoke `service-email-to-case-configure` for the source-file shape and elicitation rules. Gather routing-address inputs, optional toggle opt-outs, and Support Settings per the skill's Required Inputs — never assume the authenticated user or a customer-facing email address; ask for each explicitly.
2. **Production-org guard is the Gated checkpoint for this category.** Enabling Email-to-Case is permanent and org-wide (see step 7 below) — `apply-casesettings.py` refuses a non-sandbox/non-trial org unless `--confirm-production` is passed. Pass `--confirm-production` ONLY if the spec records that the SE already confirmed a production target for this category (per CLAUDE.md's Gated tier — confirm once per category, then autonomous); if the spec is silent and the target resolves to production, STOP, mark the item `FAILED` in your JSON output with reason "production-org confirmation required — not recorded in spec", and do not add the flag yourself.
3. Build the `CaseSettings` source file from `assets/CaseSettings.settings-meta.xml` — comment-free output, only the elements being set (toggles + routing-address fields, `enableEmailToCase` + `enableOnDemandEmailToCase` both `true`). Never include `emailAddress`, `emailServicesAddress`, or `isVerified` in the source file — the first is supplied at apply time via `--routing-email`, the other two are read-only and platform-managed.
4. Validate: `python3 scripts/validate-casesettings.py <source-file>` — resolve every reported error before applying.
5. Apply: `python3 scripts/apply-casesettings.py --target-org [alias] --input <source-file> --routing-email <email> [...one per address, in document order] [owner/automated flags ONLY if Support Settings are unset in the org] [--confirm-production per step 2]`. The script applies Phase A (Support Settings + toggles) then Phase B (routing addresses) and re-reads to verify — do not attempt a plain `deploy_metadata` in its place (a single-phase deploy fails on a fresh org; On-Demand must be live before an address binds).
6. **Verify (post-apply read-back — do NOT skip):** re-run with `--verify-only` and assert `verified.enableEmailToCase` / `verified.enableOnDemandEmailToCase` are `true`, `verified.routingAddressCount` matches the spec'd address count, and each Phase B entry is `status: created` (or `already_exists`). If any assertion fails, mark the item `FAILED` in your JSON output and surface the JSON summary verbatim in `issues`.
7. **`enableEmailToCase` cannot be turned off once applied `true`** — this is a platform behavior, not a script limitation. Note it in `discovery_notes` so the change log carries the permanence.
8. Report each routing address's exact `routingName` and generated `emailServicesAddress` so the SE can configure mail forwarding and Phase 3 can match any separately approved email-agent obligation. Do not probe Agentforce entitlement, create or modify an agent, or assign a channel in Phase 1.
9. Rollback: NOT available via this skill. `enableEmailToCase` cannot be disabled once `true` (step 7), and the apply script's routing-address merge is append-only with no delete path. De-provisioning a routing address is Setup UI / direct Metadata API only. Record this irreversibility verbatim in `rollback_commands` before applying so the SE understands it going in — this is the concrete consequence of the Gated checkpoint in step 2.
<!-- /IF:EMAIL_TO_CASE -->
