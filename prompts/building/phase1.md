...
2. **MCP note — do NOT call `salesforce-api-context` tools.** The skill's frontmatter references `get_metadata_type_sections` / `get_metadata_type_context`; those tools are NOT in Scout's MCP host. Use the skill's static `references/column-names.md` tables for platform report column names (they cover the standard objects a demo reports on — Opportunity, Case, Lead, Account). Report columns use platform names, NOT raw API field names (e.g. `CLOSE_DATE` not `CloseDate`, `STAGE_NAME` not `StageName`) — a wrong column name is this skill's #1 deployment killer, so it fails the deploy rather than producing a silent wrong result. For a column on an object/field NOT in the reference tables and not obvious from the pattern, issue one `salesforce_docs_search` (per `demo-docs-consultation`) before guessing; if still unresolved, mark the report `FAILED` and surface it in `issues` rather than deploying a guessed column.
