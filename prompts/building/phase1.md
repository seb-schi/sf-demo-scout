...
2. **Author what the skill can express, deploy it, then QA-hand-off (no complexity skip).** Author whatever `platform-flexipage-generate` supports for a `RecordPage` — header region, one/two-column `flexipage:fieldSection`s, and the skill's component catalog — and deploy it. For richer surfaces the skill covers (dynamic-form regions, tabsets, conditional-visibility rules), attempt them too; for custom-LWC placement, deploy the LWC. A FlexiPage has NO build-time render signal, so after deploy report it **"deployed — needs visual QA in App Builder"** (handover brief's *Built — Validate in Sonnet*), never "working." Only route a surface to SE Manual when `platform-flexipage-generate` genuinely cannot emit it — record that in `discovery_notes` as "flexipage surface not skill-supported: [what]". Do NOT hand-author FlexiPage XML the skill doesn't cover (risks a malformed page) and do NOT skip on "complex" grounds.
...
```json
{
  "phase": 1,
  "deployed": [
    {"type": "CustomObject|CustomField|RecordType|Layout|FlexiPage|CustomTab|CustomApplication|Queue|BusinessProcess|PathAssistant|ValidationRule|ListView|SharingRules|ReportType|ReportFolder|Report", "api_name": "string", "status": "SUCCESS|FAILED", "attempts": 1, "error": null, "lrp_section_target": "string|null — for FlexiPage type only: the field section label the deploy targeted; null otherwise"}
  ],
  "skipped": [
