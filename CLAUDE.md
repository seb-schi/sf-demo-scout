...
A **knowledge cartridge** is any installed plugin that publishes a solution-agnostic domain-knowledge layer via a stable contract: `INTEGRATING.md` + `KNOWLEDGE-INDEX.md` (with a `## Coverage` block declaring `industry` + `signals`) at its selected installation root. Scout discovers only explicitly enabled host selections, preserving provider identity; cached versions alone are not candidates. Scout consults one during sparring Stage 4 (`prompts/sparring/knowledge-cartridge.md`) when the audit's detected industry matches a cartridge's declared Coverage — read-only, proactive, no gate. The dependency points one way: **Scout reads the published contract; the cartridge never detects or depends on Scout.** A cartridge that also ships a build-executor skill routes that skill through the Stage 5 external-skills offer-gate instead, never the knowledge consult. (The Life Sciences Booster Pack is the reference implementation.)
...
- Email-to-Case base (`CaseSettings` + routing addresses via the Metadata API — `service-email-to-case-configure` skill; applied through the skill's own two-phase script, never a plain deploy. SE confirms once if the target is production — enabling it is permanent and org-wide, and cannot be turned off once on. Phase 3 separately owns any approved email-agent entitlement probe and agent creation; channel assignment stays an explicit manual/BLOCKED obligation unless an installed external skill is explicitly approved. The skill declares an `accessCheck` frontmatter field (HasEmailToCase org perm + CustomizeApplication user perm) that Scout does not enforce generically — the apply script's own fail-closed validation and the production-org guard are the practical backstop)
