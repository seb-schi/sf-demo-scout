...
- Email-to-Case (`CaseSettings` + routing addresses via the Metadata API — `service-email-to-case-configure` skill; applied through the skill's own two-phase script, never a plain deploy. SE confirms once if the target is production — enabling it is permanent and org-wide, and cannot be turned off once on. Optional Agentforce delegation probes org entitlement via `check-agent-email-capability.sh` before creating an agent. The skill declares an `accessCheck` frontmatter field (HasEmailToCase org perm + CustomizeApplication user perm) that Scout does not enforce generically — the apply script's own fail-closed validation and the production-org guard are the practical backstop)
