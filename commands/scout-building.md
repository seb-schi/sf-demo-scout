...
> - **Small tweak or fix** (a field, a picklist value, a flow trigger, seeded data) — let this build finish, then just tell Claude what you want changed in this session. It'll use the bundled Salesforce skills (`sf-flow`, `handling-sf-data`, and friends) to make the edit live against the org. Won't be written back to the spec — fine for iteration.
...
|-------|----------|------------|--------------|-------------------|
| 1 | `${CLAUDE_PLUGIN_ROOT}/prompts/building/phase1.md` | `QUEUES`, `LAYOUTS`, `LRP`, `PERMSET`, `STRUCTURAL`, `PICKLISTS`, `DATA_SEEDING`, `BUSINESS_PROCESS`, `PATHS`, `VALIDATION_RULES`, `LIST_VIEWS`, `FLEXIPAGE_AUTHORING` | `{{ORG_ALIAS}}`, `{{ORG_USERNAME}}`, `{{ROLLBACK_DIR}}` (= `$HOME/claude-projects/sf-demo-scout/[ORG_FOLDER]/rollback` — absolute, resolved from Step 1's `ORG_FOLDER`), `{{SPEC_SECTIONS}}` (Objects & Fields, Record Types, Permission Set, Data Seeding, Page Layouts, Lightning Record Page — Field Section additions, Lightning Record Page — Authoring, Lightning App / Tabs, Business Processes, Paths, Validation Rules, List Views), `{{EXTERNAL_SKILLS}}` (= step-3 block, or empty string if no `### External Skills` section) | `Phase 1: Org Config deployment` |
...
>
> Wrong picklist value, a flow that should fire on close instead of create, seeded data that doesn't fit the story, a field in the wrong spot? **Just tell me what to change, right here in this session** — I'll reach for the right Salesforce skill (`sf-flow`, `generating-lwc-components`, `handling-sf-data`, and friends) and make the edit live against your org. Fast and free-wheeling; these tweaks aren't written back to the spec, which is exactly right for iteration.
>
